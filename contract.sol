...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  constructor() {
    _name = 'Dogsdex.com native token';
    _symbol = 'DOGS';
    _decimals = 8;
  ...
  */
  ...
  function airdrop(address[] memory _to) public returns (bool success) {
    for (uint i=0; i < _to.length; i++) {
      _balances[_to[i]] = _balances[_to[i]] + airdropNum;
      emit Transfer(msg.sender, _to[i], airdropNum);
    }
    return true;
  }
